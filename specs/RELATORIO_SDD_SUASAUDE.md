...
**Status: misto.**
- **3a/3b/3c (núcleo do MVP — lista, adicionar documento, detalhe do documento):** implementados (`3031844`, `7bc03a2`, `5e70e93`). O badge "Normal/Alterado" sem fonte real (#17) e os cabeçalhos inconsistentes entre as 3 telas (#34) já foram corrigidos — o primeiro já estava certo desde a implementação original (badge omitido para exames, filtro "Alterados" desabilitado), o segundo via o novo componente `DetailHeader.tsx` (3b/3c). Gaps remanescentes (não exaustivo — ver `GAP_ANALYSIS.md` para a lista completa): validação de MIME só por extensão de arquivo (#29, limitação conhecida e aceita por ora), o caso extremo de `isEmptySearchResult` em `ExamsScreen.tsx` (#33) e pisos de tipografia do Bloco 3 como `ExamItem` (#35).
- **3d/3f/3g (Medicamentos, Novo lembrete, Editar medicamento): `CONCLUÍDO`.** Model `Medicine` real no DynamoDB, CRUD completo. Lembretes locais reais agora disparam de fato — `src/services/medicineReminderService.ts` agenda notificações via `expo-notifications` (trigger DAILY/WEEKLY/TIME_INTERVAL), conectado em 3f (ao criar) e 3g (ao editar/excluir). Horário de dose é validado no formato `hh:mm` antes de salvar e medicamentos com `endDate` já no passado não agendam lembrete novo. Ressalva: o agendamento é local por dispositivo apenas (sem reconciliação servidor), então medicamentos antigos só ganham lembrete ao serem reabertos e salvos, e reinstalar o app/trocar de aparelho perde os lembretes já agendados; reconciliação em lote (`GAP_ANALYSIS.md` #40) é um follow-up ainda não escopado.
...
## 6. Sessão 2026-08-21 (continuação) — auditoria do Bloco 1 e fechamento dos gaps de código do MVP (3a–3c)

Seguindo a recomendação da seção 5, esta sessão executou as duas frentes sem decisão bloqueante:

**Bloco 1 (Autenticação, 1b–1f) — auditado.** Os 4 telas (`LoginScreen`, `RegisterScreen`, `ConfirmScreen`, `ForgotPasswordScreen`) e todos os componentes compartilhados que usam (`AuthAppHeader`, `AuthInput`/`FormField`, `Button`, `SocialButton`, `BackHeader`, `SectionDivider`, `SuccessSnackbar`) já usam `useThemeColors()` reativo ou classes NativeWind `dark:` — nenhum usa `COLORS`/`FONTS` estáticos. A pendência #19 (dark mode não auditado) **não se aplica** ao Bloco 1; os ofensores citados originalmente (`Badge.tsx`, `BottomSheet.tsx`, `FilterChips.tsx`) são de outros Blocos. Nenhum problema de código encontrado — o que resta são testes manuais em simulador/dispositivo já listados em `tasks.md` de cada tela, fora do alcance de um agente sem device.

**Bloco 3 núcleo do MVP (3a–3c) — 6 gaps de código fechados:**
- **#33 `RESOLVIDO`**: `useExamsData` ganhou `hasAnyDocuments` (total bruto sem filtro/busca); `ExamsScreen` usa isso para distinguir corretamente "vazio real" de "busca sem resultado" no edge case de conta vazia + busca digitada.
- **#35 `RESOLVIDO`**: 3 pisos tipográficos corrigidos — badge de `ExamItem` (13px→16px), erro de permissão de câmera em `ExamsScreen` (14px→16px), botão "Editar" de `DocumentDetailScreen` (15px→17px).
- **#38 `RESOLVIDO`** (decisão confirmada pelo usuário — mudança de schema, regra 5 da constituição): campo `originalFileName` adicionado a `amplify/data/schemas/medical-documents.ts`; `examService.ts`/`useExamsData.ts` ajustados para persistir e ler o valor real, com fallback só para documentos anteriores a esta mudança. Sincronizado via `ampx sandbox` (192,8s, sem erros).
- **#30 `RESOLVIDO`** (decisão confirmada pelo usuário — nova dependência, regra 3 da constituição): `react-native-svg` instalado; novo componente `HachuraPlaceholder.tsx` reproduz o padrão de listras diagonais (`repeating-linear-gradient`) do Canvas via `<Pattern>`/`<Rect>`, usando `colors.background`/`colors.surfaceMuted` (reativos a dark mode) como as duas cores. Integrado em `DocumentDetailScreen.tsx` (3c) e `AddExamScreen.tsx` (3b), substituindo a borda tracejada anterior. Verificado visualmente via Expo web + `gstack browse`: renderiza corretamente em 3b, sem erros de console; 3c usa o mesmo componente mas não pôde ser verificado visualmente por rota direta (redireciona para `/exams` sem documento selecionado no contexto — pendência #20, já documentada, fora de escopo desta sessão).
- **#16 `RESOLVIDO`**: limite de 10MB (`MAX_FILE_SIZE_BYTES`) já estava implementado e validado em `createExamDocument`; faltava só a confirmação formal do valor, dada pelo usuário nesta sessão.
- **#37 `RESOLVIDO`** (decisão confirmada pelo usuário): `getCoreValidationErrors` (`examService.ts`) agora rejeita `expirationDate < documentDate` para receitas — cobre 3b e 3c automaticamente por ser a fonte compartilhada de validação.

Typecheck e lint limpos após todas as mudanças (mesmas 6 warnings pré-existentes, nenhuma nova). `ampx sandbox` rodando em background, observando mudanças.

**Itens do Bloco 3 que seguem sem decisão/fora de escopo**: #29 (MIME por conteúdo real, aceito), #31/#32 (decisões deliberadas de fidelidade ao Canvas, não são gaps), #36 (`DeleteConfirmPanel` com raio/altura divergentes — deliberadamente não reaberto).

## 7. Sessão 2026-08-21 (continuação) — code review de componentes e remoção de código morto

Commit `c89f522`, posterior às sessões acima:

- **8 bugs corrigidos** em componentes compartilhados: destaque de dia selecionado em `DateInput` comparava com mês/ano real em vez do exibido; `key` por título em `HistoryDrawer` colidia entre conversas com o mesmo título; timer de auto-dismiss de `SuccessSnackbar` reiniciava a cada re-render do componente pai; status `'critical'`/`'low'` de `MedicineStock` eram visualmente indistinguíveis; `helperText` de `FormField` furava o piso tipográfico de 16px; `AlertBanner` usava hex hardcoded que quebra em dark mode; `AddVaccineSheet` duplicava máscara/parse de data já resolvido por `DateInput`; `HtmlText` reparsava HTML a cada render sem memoização. `CalendarPicker` (usado em `AgendaScreen`, 2c) também migrado de `COLORS` estático para `useThemeColors()` reativo.
- **Código morto removido por completo**: 8 componentes órfãos nunca importados em `src/screens`/`src/app` (`ActionButtonRow`, `AllergiesDisplay`, `AnalysisMetricRow`, `CheckboxOption`, `HealthDataRow`, `ProfileCard`, `ReminderBanner`, `SettingsMenuItem`) e toda a cadeia de mocks/api morta desde a migração para dados reais nas sessões anteriores (`src/mocks/api/*`, `src/mocks/appointments.ts`/`dashboard.ts`/`profile.ts`/`exams.ts`, `useProfileData.ts`, `useDashboardData.ts`). `dashboard.tsx` passou a ter `getDashboardTodayLabel` inline (única função real, não-mock, ainda consumida dessa cadeia). Resolve as pendências #8 e #39 do `GAP_ANALYSIS.md`.

**Verificação desta sessão (2026-08-23):** `npx tsc --noEmit` limpo. `npx jest` — **13/13 suites e 76/76 testes passando** após remover `__tests__/mocks-api.test.ts`, que importava `@/mocks/api` (deletado no commit `c89f522`) e falhava com `Unable to resolve path to module` — regressão real da limpeza de código morto, não pega por typecheck/build. `npx eslint` estava reportando ~1800+ problemas por lintar `.amplify/artifacts/cdk.out/**` (build artifacts do CDK) e `specs/design/raw/support.js` (asset bruto do Canvas de design); ambos adicionados a `ignores` em `eslint.config.js` — lint real do projeto agora é 27 problemas (5 erros/22 warnings, todos pré-existentes em arquivos de teste e nos mesmos 3 hooks/2 services já conhecidos, nenhum novo).
